...
if (config.ShowBuildOptions) {
  ...
  console.log(`[SPT Build System] DeleteDistAfterRunning:     ${config.DeleteDistAfterRunning}`);
...
if (config.BuildZip) {
  ...
  zip({
    source: "*",
    destination: `${currentDir}\\${config.BuildDir}\\${modName}-${version ?? ""
      }.zip`,
    ...
    .then(function () {
      ...
      if (config.DeleteDistAfterRunning) {
        fs.rmSync(`${currentDir}/${config.BuildDir}/user/`, {
          force: true,
          recursive: true,
        });
        console.log(
          chalk.yellow("[SPT Build System] Removed dist content after running.")
        );
      }
...
if (!config.BuildZip && config.DeleteDistAfterRunning) {
  fs.rmSync(`${currentDir}/${config.BuildDir}/user/`, {
    force: true,
    recursive: true,
  });
